...
const defaultState = { phase: 'lobby', p0: null, p30: null, chgPct: 0, nowIdx: MID_INDEX, startedAt: null, participants: {}, winner: null };
function read() {
    try {
...
async function mockJoin(uuid, name) {
    const s = read();
...
export const mock = {
    ...
    resolve() { const s = read(); s.phase = 'results'; s.chgPct = (Math.random() * 10 - 5) / 100; if (!s.p0) {
        s.p0 = 62000 + (Math.random() * 2000 - 1000);
    } s.p30 = s.p0 ? (s.p0 * (1 + s.chgPct)) : null; const idx = clampIdx(s.nowIdx); s.nowIdx = idx; s.winner = { ball: LANES[Math.round(idx)] }; s.startedAt = null; write(s); },
    addRandom() { const uuid = crypto.randomUUID?.() || Math.random().toString(36).slice(2); return mockJoin(uuid); },
    seed(n) { for (let i = 0; i < n; i++)
        this.addRandom(); }
};
// small ticker to move index during live
setInterval(() => { const s = read(); if (s.phase === 'live') {
    const dir = Math.random() > 0.5 ? 1 : -1;
    let step = Math.random() * 1.8 * dir;
    s.nowIdx = clampIdx(s.nowIdx + step);
    write(s);
} }, 100);
